...
**Expected:** Returns complete quote with `origin`/`destination` in response (from rate)
...
```json
{
  ...
  "data": [
    {
      ...
      "earliest_departure": {...}  // ✅ If requested (message included if not found)
    }
  ],
  "metadata": {
    "api_version": "v4",
  ...
  "message": "No rates found for cargo_ready_date ..." // Only when no results
}
```
...
```json
{
  ...
  "data": {
    ...
    "inland_haulage": {...},       // ✅ If inland detected
    "earliest_departure": {...},   // ✅ If requested (message included if not found)
    "upcoming_departures": [...],  // ✅ Next 4 sailings (present when schedules exist)
    "metadata": {
      "cargo_ready_date": "2025-11-18",
      ...
    }
...
- [ ] cargo_ready_date filters rates and schedules correctly
